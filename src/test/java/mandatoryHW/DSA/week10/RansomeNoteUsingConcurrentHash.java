...
public class RansomeNoteUsingConcurrentHash 
{
	/* Time - 
	 * Space - 
	...
	{
		ConcurrentHashMap<Character, Integer> conCurMap = new ConcurrentHashMap<Character, Integer>();
		for (int i = 0; i < magazine.length(); i++) 
		{
			conCurMap.put(magazine.charAt(i), conCurMap.getOrDefault(magazine.charAt(i), 0)+1);
			conCurMap.put(ransomNote.charAt(i), conCurMap.getOrDefault(ransomNote.charAt(i), 0)-1);
		}
		for (Entry<Character, Integer> entry : conCurMap.entrySet()) 
		{
			if (entry.getValue() == -1) {
				return false;
			}
